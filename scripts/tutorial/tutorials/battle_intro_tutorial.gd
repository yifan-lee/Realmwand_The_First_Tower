...
func _on_ui_confirmed() -> void:
	...
	match current_step:
		Step.INTRO:
			...
			_ui.show_prompt_at("这是你的 HP（生命），归零则战斗失败。", target, true)
		...
		Step.HP:
			...
			_ui.show_prompt_at("ATK（攻击）决定了你的基础物理伤害。", target, true)
		...
		Step.DEF:
			...
			_ui.show_prompt_at("SPD（速度）决定 ATB 读条速度。当标记到达右侧即可行动！\n每次行动需要花费的时间是 100 / spd", target, true)
